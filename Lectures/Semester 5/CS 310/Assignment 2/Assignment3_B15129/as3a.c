...
int main(int argc, char const *argv[])
{
	...
	{	
		printf("Enter the string in which pattern to be matched : \t");
		//getline(&string,&Max_Len2,stdin);
		scanf("%s",string);
		matched(string,pattern,i,"No_File");
	}
	// printf("wesx\n");
	//printf("%s\t%s\n",string,pattern);
	// printf("\n\n\n\n\n\n%d\n\n\n\n\n",i );
 ...
 //         args[j] = (char *)malloc(20 * sizeof(char));
	...
	// for (j = 0; j < 72; ++j)
	// {
	// 	sprintf(args[j],"Out/output%d.txt",j+1 );
	// }
	// args[j]=NULL;
	// pid_t exep;
	// exep = fork();
